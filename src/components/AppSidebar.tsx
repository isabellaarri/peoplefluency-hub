import {
  ...
  RefreshCw, GitBranch, LogOut, UsersRound,
...
export function AppSidebar() {
  ...
  const roleColor = isAdmin
    ...
    : isLeader
    ? "bg-fluency-purple-light/20 text-primary"
  ...
  return (
    ...
    >
      {/* Logo header — Fluency deep purple #320261 */}
      <div
        className="flex h-14 items-center gap-2.5 overflow-hidden px-3.5"
        style={{ background: "hsl(272 96% 19%)" }}
      >
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-white/15 text-xs font-extrabold text-white tracking-tight">
          F
        </div>
        <div
        ...
        >
          ...
          <p className="text-[9px] font-medium text-white/60 tracking-widest uppercase mt-0.5">
            People
          </p>
        </div>
      </div>
        ...
        {sections
          ...
          .map((section, si) => (
            <div key={section.label}>
              ...
              <p
                className={`px-2.5 py-1.5 text-[9px] font-semibold uppercase tracking-widest text-sidebar-muted transition-all duration-200 ${
                  expanded ? "opacity-100" : "opacity-0 h-0 py-0"
                }`}
              >
                {section.label}
              </p>
      ...
      <div className="border-t border-sidebar-border px-2 py-2 space-y-0.5">
        ...
        >
          ...
          >
            ...
            <span
              className={`text-[9px] px-1.5 py-0 rounded w-fit font-semibold mt-0.5 ${roleColor}`}
            >
              {roleLabel}
            </span>
          </div>
